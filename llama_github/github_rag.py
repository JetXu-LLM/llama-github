...
class GithubRAG:
    ...
    async def answer_with_context(self, query: str, contexts: Optional[List[str]] = None) -> Any:
        """
        Generate an answer based on the user's query and an optional list of context strings.

        Args:
            query (str): The user's question or query.
            contexts (Optional[List[str]]): A list of context strings. If None, context will be retrieved.

        Returns:
            The direct answer or response generated by the rag_processor.llm_handler.
        """
        pass
...
class GithubRAG:
    ...
    async def answer_with_context(self, query: str, contexts: Optional[List[str]] = None) -> Any:
        try:
            if contexts is None:
                # Assuming `retrieve_context` returns a list of strings as contexts
                contexts = await self.retrieve_context(query)

                if not contexts:
                    logger.error("Repo search retrieved successfully.")

                answer = await self.rag_processor.llm_handler.ainvoke(query, contexts)
                return answer
